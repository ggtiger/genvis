...
export function findChromeExecutable(): string | undefined {
  const override = process.env.BROWSER_CDP_CHROME_PATH?.trim();
...
export function getDefaultProfileDir(): string {
  ...
  return path.join(base, 'browser-cdp-profile');
}
...
export async function launchChrome(url: string, profileDir?: string): Promise<{ cdp: CdpConnection; chrome: ReturnType<typeof spawn> }> {
  ...
  if (!chromePath) throw new Error('Chrome not found. Set BROWSER_CDP_CHROME_PATH env var.');
...
export async function screenshot(session: ChromeSession, options?: { format?: 'png' | 'jpeg'; quality?: number }): Promise<string> {
  const result = await session.cdp.send<{ data: string }>('Page.captureScreenshot', {
...
export async function navigate(session: ChromeSession, url: string): Promise<void> {
