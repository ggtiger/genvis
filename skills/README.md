...
Genvis skills are Claude SDK plugins that extend AI capabilities with specialized tools.
...
**template.json is the primary config file for Genvis**, used for both skills and apps.

| File | Purpose | When to Use |
|------|---------|-------------|
| `template.json` | Genvis primary config | Always preferred. Supports all fields (metadata, envVars, etc.) |
...
- `template.json`: Genvis's extended config. Supports additional fields like `envVars`, `category`, `tags`. Takes priority for all metadata fields.
...
Contains Genvis-specific extended fields:
...
1. Open Genvis -> Workspace tab -> Skills section
