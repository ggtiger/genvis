...
# Tests the packaged Genvis.app for basic functionality
...
cleanup() {
    if [ -n "$APP_PID" ]; then
        ...
        pkill -f "Genvis.app" 2>/dev/null || true
        sleep 2
    fi
}
...
if pgrep -f "Genvis.app" > /dev/null; then
    APP_PID=$(pgrep -f "Genvis.app" | head -1)
...
while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
    ...
    if [ $ATTEMPT -ge $MAX_ATTEMPTS ]; then
        ...
        LOG_PATH="$HOME/Library/Logs/Genvis"
        if [ -d "$LOG_PATH" ]; then
...
echo "  4. When done, run: pkill -f 'Genvis.app'"
echo ""
