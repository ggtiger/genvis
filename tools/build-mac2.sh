...
echo -e "\033[0;36m  Genvis macOS Build Script v3.0\033[0m"
echo -e "\033[0;36m============================================\033[0m"
echo ""
...
# Step 5.5: Pre-build Skills (auto-detect from template.json "prebuild": true)
echo ""
echo -e "\033[0;33m========================================\033[0m"
echo -e "\033[0;33mStep 5.5/8 : Pre-build Skills\033[0m"
echo -e "\033[0;33m========================================\033[0m"
echo ""

info "Auto-detecting skills with prebuild=true in template.json..."
info "Running: node scripts/prebuild-skills.js"
if node scripts/prebuild-skills.js; then
    success "Skills pre-build step completed"
else
    error "Skills pre-build failed"
    exit 1
fi

# Clean .next/ from non-prebuilt skills to avoid bloating the package
info "Cleaning non-prebuilt skills..."
node scripts/clean-nonprebuilt-skills.js
success "Non-prebuilt skills cleaned"
...
if [ "$SKIP_TEST" = false ]; then
    ...
    if [ -d "dist/mac-$ARCH/Genvis.app" ]; then
        info "Found packaged app: dist/mac-$ARCH/Genvis.app"

        # Test 1: Launch application
        info "Test 1: Launching application..."

        open "dist/mac-$ARCH/Genvis.app" &
        APP_PID=$!
        ...
        pkill -f "Genvis.app" || true
        sleep 2

        success "Automated testing completed"

    elif [ -d "dist/mac/Genvis.app" ]; then
        # Fallback to old path structure
        info "Found packaged app: dist/mac/Genvis.app"
        warning "Note: Using legacy dist path structure"
        # Run similar tests...
    else
        warning "Packaged app not found at expected locations:"
        warning "  - dist/mac-$ARCH/Genvis.app"
        warning "  - dist/mac/Genvis.app"
        if [ -d "dist" ]; then
...
if [ -d "dist" ]; then
    ...
    if [ -d "dist/mac-$ARCH/Genvis.app" ]; then
        APP_SIZE_MB=$(du -sm "dist/mac-$ARCH/Genvis.app" | cut -f1)
        echo -e "  - Genvis.app ($APP_SIZE_MB MB)"
    elif [ -d "dist/mac/Genvis.app" ]; then
        APP_SIZE_MB=$(du -sm "dist/mac/Genvis.app" | cut -f1)
        echo -e "  - Genvis.app ($APP_SIZE_MB MB)"
    fi
...
echo -e "  1. Manual test: open dist/mac*/Genvis.app"
echo -e "  2. Install test: mount dist/Genvis-*.dmg"
