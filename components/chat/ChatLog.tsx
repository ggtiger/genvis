...
const ToolResultMessage = ({
  ...
  // ====== CodeBlock: copy + confirm-execute for code blocks ======
    ...
    return React.Children.toArray(node)
      .map(child => {
        ...
        if (React.isValidElement(child)) {
          ...
          if (p?.children) return extractTextFromChildren(p.children as React.ReactNode);
        }
        return '';
      })
      .join('')
  ...
  const SHELL_LANGS = ['bash', 'sh', 'shell', 'zsh', 'fish', 'powershell', 'cmd', 'terminal'];

  const isShellCode = (code: string, lang?: string): boolean => {
    if (lang && SHELL_LANGS.includes(lang.toLowerCase())) return true;
    const first = (code.split('\n')[0] ?? '').trim();
    return /^(npm|yarn|pnpm|npx|git|docker|kubectl|python|python3|node|pip|cargo|go|cd|ls|pwd|mkdir|rm|cp|mv|cat|echo|curl|wget|brew|apt|yum|dnf|pacman|make|chmod|chown|ssh|scp|rsync|tar|zip|unzip)\s/.test(first)
      || /^\$\s+/.test(first);
  };

  const copyText = async (text: string): Promise<boolean> => {
    try {
      if (typeof navigator !== 'undefined' && typeof navigator.clipboard?.writeText === 'function' && document.hasFocus()) {
  ...
  const runInTerminal = async (cmd: string): Promise<boolean> => {
    try {
      // Use the same session ID as the right-side terminal: `cli-${projectId}`
      const sid = `cli-${projectId}`;
      const BASE = process.env.NEXT_PUBLIC_API_BASE ?? '';
      const cr = await fetch(`${BASE}/api/terminal/${sid}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'create' }),
      });
      if (!cr.ok) throw new Error('session failed');
      const wr = await fetch(`${BASE}/api/terminal/${sid}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'input', data: cmd.endsWith('\n') ? cmd : `${cmd}\n` }),
      });
      return wr.ok;
    } catch (e) { console.error('[CodeBlock] exec error:', e); return false; }
  };
  ...
  const CodeBlock = ({ children }: { children: React.ReactNode }) => {
    ...
    const isCmd = isShellCode(code, language);

    const handleCopy = async () => {
      if (await copyText(code)) { setCopied(true); setTimeout(() => setCopied(false), 2000); }
    };

    const handleExec = async () => {
      const ok = await runInTerminal(code);
      if (ok) { setExecuted(true); setTimeout(() => setExecuted(false), 2000); }
    };
    ...
    return (
      <div className="relative group my-2 rounded-lg overflow-hidden border border-white/15 dark:border-white/[0.06]">
        ...
        <div className="flex items-center justify-between px-3 py-1.5 bg-gray-800 text-slate-300 dark:text-slate-600">
          <span className="text-[11px] font-mono">{language || (isCmd ? 'shell' : 'code')}</span>
          <div className="flex items-center gap-1">
            ...
            {isCmd && (
              <button onClick={handleExec} className="px-2 py-0.5 rounded text-[11px] bg-green-700/80 hover:bg-green-600 text-white flex items-center gap-1 transition-colors" title="在终端执行">
                {executed ? <Check size={12} /> : <TerminalIcon size={12} />}
                <span>{executed ? '已发送' : '执行'}</span>
              </button>
            )}
          </div>
        </div>
  ...
  // Reusable markdown components factory
  const mdComponents = (extraP?: (c: React.ReactNode) => React.ReactElement | null) => ({
    p: extraP
      ? ({children}: any) => extraP(children) ?? <p className="mb-2 last:mb-0 break-words">{children}</p>
      : ({children}: any) => <p className="mb-2 last:mb-0 break-words">{children}</p>,
    strong: ({children}: any) => <strong className="font-medium">{children}</strong>,
    em: ({children}: any) => <em className="italic">{children}</em>,
    code: ({children, className}: any) => (
      <code className={`bg-white/15 dark:bg-white/5 px-1.5 py-0.5 rounded text-[13px] font-mono text-pink-600 ${className || ''}`}>{children}</code>
    ),
  ...
  const planningPHandler = (children: React.ReactNode) => {
    ...
    if (arr.some(c => typeof c === 'string' && c.includes('Planning for next moves...'))) {
      return <p className="mb-2 last:mb-0 break-words"><code className="bg-white/15 dark:bg-white/5 px-2 py-1 rounded text-xs font-mono">Planning for next moves...</code></p>;
    }
    return null;
  };
  ...
  const getMessageTypeInfo = (message: ChatMessage) => {
    ...
    switch (messageType) {
      case 'tool_result':
        return {
          bgClass: 'bg-blue-500/10 dark:bg-blue-500/20 border border-blue-200/50 dark:border-blue-500/20 ',
      ...
      default:
        ...
        switch (role) {
          case 'user':
            return {
              bgClass: 'bg-white border border-white/15 dark:border-white/[0.06] ',
        ...
        >
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              ...
              className="text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300 text-xl"
            >
              ✕
            </button>
          </div>
          ...
          <div className="space-y-4">
            <div className="text-slate-800 dark:text-white ">
              <strong className="text-slate-700 dark:text-slate-300 ">Type:</strong> {type}
            </div>
            <div className="text-slate-800 dark:text-white ">
              <strong className="text-slate-700 dark:text-slate-300 ">Time:</strong> {formatTime(selectedLog.timestamp)}
            </div>

            {type === 'tool_result' && data.diff_info && (
              <div>
                <strong className="text-slate-700 dark:text-slate-300 ">Changes:</strong>
                <pre className="bg-white/15 dark:bg-white/5 p-3 rounded-lg overflow-x-auto text-xs font-mono">
                  {data.diff_info}
                </pre>
              </div>
            )}
            ...
            <div>
              <strong className="text-slate-700 dark:text-slate-300 ">Detailed Data:</strong>
              <pre className="bg-white/15 dark:bg-white/5 p-3 rounded-lg overflow-x-auto text-xs font-mono">
      ...
      <div className="flex-1 overflow-y-auto px-8 py-3 space-y-2 custom-scrollbar ">
                ...
                {message.role === 'user' ? (
                  ...
                  <div className="w-full">
                    {message.messageType === 'tool_result' ? (
                      ...
                      <div className="text-sm text-slate-800 dark:text-white leading-relaxed">
                        ...
                        {pendingPlanApproval && ensureStableMessageId(message) === pendingPlanApproval.messageId && !pendingPlanContent && (
                          <div className="mt-3 flex gap-2">
                            ...
                            <button
                              className="px-3 py-1 bg-gray-200 text-slate-800 dark:text-white rounded"
                              onClick={() => {
                                setPendingPlanApproval(null);
        ...
        {pendingPlanContent && pendingPlanApproval && (
          <div className="mt-6 w-full">
            <div className="p-4 border border-gray-300 rounded bg-white/5 dark:bg-white/5">
              <div className="prose prose-sm max-w-none text-slate-800 dark:text-white">
                                <ReactMarkdown components={mdComponents()}>{pendingPlanContent}</ReactMarkdown>
              </div>
              <div className="mt-3 flex gap-2">
                <button
                  className="px-3 py-1 bg-black text-white rounded"
        ...
        {conversationStats && !isWaitingForResponse && (
          <div className="mb-4 w-full pl-1">
            <div className="text-[10px] text-slate-400 dark:text-slate-500 flex flex-wrap items-center gap-x-1">
              {conversationStats.duration_ms !== undefined && (
                <>
                  ...
                  {conversationStats.duration_api_ms !== undefined && (
                    <span className="text-slate-300 dark:text-slate-600">(API {(conversationStats.duration_api_ms / 1000).toFixed(1)}s)</span>
                  )}
                </>
              )}
              ...
              {conversationStats.usage && (
                <>
                  ...
                  {conversationStats.usage.cacheReadInputTokens && conversationStats.usage.cacheReadInputTokens > 0 && (
                    <span className="text-slate-300 dark:text-slate-600">({((conversationStats.usage.cacheReadInputTokens) / 1000).toFixed(1)}K cached)</span>
                  )}
                </>
              )}
