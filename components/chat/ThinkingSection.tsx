...
export default function ThinkingSection({ 
  ...
  const formatThinkingContent = (text: string) => {
    ...
    return parts.map((part, index) => {
      ...
      if (index % 2 === 1) {
        return (
          <span key={index} className="font-medium text-slate-600 dark:text-slate-400 ">
            {part}
          </span>
        );
      }
  ...
  return (
    <div className="my-2 text-sm text-slate-500 dark:text-slate-400 ">
      {/* Always visible first line */}
      <div 
        onClick={() => hasMoreContent && setIsExpanded(!isExpanded)}
        className={`${hasMoreContent ? 'cursor-pointer hover:text-slate-600 dark:text-slate-400 ' : ''} transition-colors`}
      >
      ...
      {hasMoreContent && (
        <AnimatePresence>
          {isExpanded && (
            ...
            >
              <div className="italic text-slate-500 dark:text-slate-400 leading-relaxed whitespace-pre-wrap">
