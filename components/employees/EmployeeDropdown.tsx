...
export default function EmployeeDropdown({
  ...
  // Only fetch if no external employees provided
  useEffect(() => {
    ...
    async function loadEmployees() {
      try {
        ...
        if (response.ok) {
          ...
          setInternalEmployees(data.data || []);
        }
      } catch (error) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        {(() => { const Icon = getAvatarIcon(selectedEmployee?.avatar); return <Icon className="w-4 h-4 text-gray-400" />; })()}
            ...
            employees.map((employee) => (
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  {(() => { const Icon = getAvatarIcon(employee.avatar); return <Icon className="w-8 h-8 text-gray-400 flex-shrink-0" />; })()}
