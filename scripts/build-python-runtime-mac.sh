...
# Remove third-party packages from site-packages (keep only pip and setuptools)
SITE_PACKAGES="$TARGET_DIR/lib/python3.12/site-packages"
if [ -d "$SITE_PACKAGES" ]; then
    find "$SITE_PACKAGES" -mindepth 1 -maxdepth 1 ! -name 'pip' ! -name 'setuptools' ! -name '_distutils_hack' ! -name 'distutils-precedence' ! -name 'README.txt' -exec rm -rf {} + 2>/dev/null || true
    echo "  - Removed third-party packages from site-packages"
fi
