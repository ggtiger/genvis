...
禁止在 Python CLI 脚本的 print() 输出中使用 emoji 和 Unicode 特殊符号（✓✗📊🟢⚪•└─等），改用纯 ASCII 文本标签如 [OK][ERROR][STATUS]，确保 Windows GBK 编码兼容。
所有面向终端输出的脚本必须遵守 ASCII-safe 原则，UI 界面可正常使用 emoji。
