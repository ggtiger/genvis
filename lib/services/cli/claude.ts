...
export async function executeClaude(
  ...
  // Hook-based feedback detection: set by PostToolUse when AskUserQuestion tool is invoked
  let askUserQuestionDetected = false;
  let askUserQuestionContent: string | undefined;
  ...
  try {
    ...
    // work, boss, cli 模式不需要检查 projectType
    if (projectMode !== 'work' && projectMode !== 'boss' && projectMode !== 'cli' && !projectType) {
      throw new Error('项目类型未定义：projectType 字段缺失');
    }

    // work/boss/cli 模式使用专用提示词
    let systemPromptText: string;
    if (projectMode === 'work' || projectMode === 'boss' || projectMode === 'cli') {
      console.log(`[ClaudeService] ✅ Using ${projectMode.toUpperCase()} mode prompt`);
      ...
      if (projectMode === 'boss') {
        // Boss模式：直接使用员工的系统提示词，不添加文件操作相关的前缀
        const { getPrompt } = await import('@/lib/config/prompts');
        const basePrompt = employeePrompt || await getPrompt('boss-mode');
        systemPromptText = basePrompt;
      } else if (projectMode === 'cli') {
        // CLI模式：只提供命令行建议，不执行任何工具
        const { getPrompt } = await import('@/lib/config/prompts');
        const basePrompt = employeePrompt || await getPrompt('cli-mode');
        systemPromptText = basePrompt;
      } else {
        // Work模式：使用buildExecutionSystemPrompt添加文件操作相关的前缀
...
**禁止执行任何删除操作。** 如用户要求清理文件，请使用移动到指定文件夹（如 _trash）的方式替代删除。`;
      ...
      // 检查是否有员工专属的 system_prompt_execution（优先于通用项目类型提示词）
      const employeeId = (project as any).employee_id as string | undefined;
      let employeeExecutionPrompt: string | undefined;

      if (employeeId) {
        const { getEmployeeById } = await import('@/lib/services/employee-service');
        const employee = await getEmployeeById(employeeId);
        if (employee?.system_prompt_execution?.trim()) {
          employeeExecutionPrompt = employee.system_prompt_execution;
          console.log(`[ClaudeService] 👤 Using employee execution prompt for: ${employee.name}`);
        }
      }

      if (employeeExecutionPrompt) {
        // 使用员工专属执行提示词，包裹在 buildExecutionSystemPrompt 中以获得文件操作前缀
        const { buildExecutionSystemPrompt } = await import('@/lib/config/prompts');
        const { loadGlobalSettings } = await import('@/lib/services/settings');
        const settings = await loadGlobalSettings();
        systemPromptText = buildExecutionSystemPrompt(absoluteProjectPath, employeeExecutionPrompt, settings.ai_services);
        console.log(`[ClaudeService] 🎯 Using employee-specific execution prompt (${systemPromptText.length} chars)`);
      } else {
    ...
    const { getEnabledSkillPaths, getSkillEnvVars, getAllSkillsWithStatus } = await import('@/lib/services/skill-service');
    ...
    // Inject env vars from all enabled skills into the subprocess environment
    try {
      const allSkills = await getAllSkillsWithStatus();
      ...
      for (const skill of enabledSkills) {
        ...
        if (Object.keys(skillEnv).length > 0) {
          ...
          console.log(`[ClaudeService] 🔑 Injected env vars from skill "${skill.name}":`, Object.keys(skillEnv));
        }
      }
    } catch (error) {
      console.error('[ClaudeService] Failed to inject skill env vars:', error);
    }
    ...
    const canUseTool = async (toolName: string, toolInput: Record<string, unknown>) => {
          try {
            ...
            // When SDK stream closes while waiting for permission, allow by default to avoid blocking
    ...
    const preToolUseHook = async (input: any, toolUseID: string) => {
      try {
        ...
        // CLI mode: deny ALL tools - advisory only mode
        if (projectMode === 'cli') {
          console.log(`[ClaudeService] 🚫 PreToolUse: ${toolName} denied (CLI advisory mode)`);
          return { decision: 'deny', reason: 'CLI模式下禁止执行任何工具，只提供命令行建议' };
        }
        ...
        // When SDK stream closes while waiting for permission, this catch prevents the error from propagating
        console.warn(`[ClaudeService] Error in PreToolUse hook (stream likely closed):`, (error as Error).message);
        return {};
      }
    };
    ...
    const postToolUseHook = async (input: any, toolUseID: string) => {
      try {
        ...
        // Detect AskUserQuestion tool - structured feedback signal from SDK
        if (toolName === 'AskUserQuestion') {
          askUserQuestionDetected = true;
          // Extract question content from structured tool input
          const questions = toolInput.questions;
          if (Array.isArray(questions) && questions.length > 0) {
            askUserQuestionContent = questions.map((q: any) => q.question || '').filter(Boolean).join('\n');
          }
          console.log(`[ClaudeService] 🔔 AskUserQuestion detected via PostToolUse hook | id=${toolUseID}`);

          // Publish structured question to frontend via SSE
          streamManager.publish(projectId, {
          ...
          timelineLogger.logSDK(
            projectId,
            `AskUserQuestion detected via hook: ${askUserQuestionContent?.substring(0, 100) || 'no content'}`,
            'info', requestId, { toolInput }, 'sdk.ask_user_question'
          ).catch(() => { });
        }
        ...
        // Dispatch tool result message with error handling
        try {
          await dispatchToolMessage({
            projectId,
            metadata,
          ...
          console.warn(`[ClaudeService] Failed to dispatch PostToolUse message for ${toolName}:`, dispatchError);
        }

        return {};
      } catch (error) {
        console.warn(`[ClaudeService] Error in PostToolUse hook:`, error);
        return {};
      }
    };
    ...
    const postToolUseFailureHook = async (input: any, toolUseID: string) => {
      try {
        ...
        // Dispatch tool failure message with error handling
        try {
          await dispatchToolMessage({
            projectId,
            metadata,
          ...
          console.warn(`[ClaudeService] Failed to dispatch PostToolUseFailure message for ${toolName}:`, dispatchError);
        }

        return {};
      } catch (error) {
        console.warn(`[ClaudeService] Error in PostToolUseFailure hook:`, error);
        return {};
      }
    };
    ...
    // Feedback detection: prefer SDK hook-based AskUserQuestion signal over text heuristics
    let feedbackDetected = false;
    if (requestId) {
      if (askUserQuestionDetected) {
        // Primary: AskUserQuestion tool was invoked — definitive structured signal
        try {
          const { markUserRequestAsWaitingFeedback } = await import('@/lib/services/user-requests');
          const feedbackContent = askUserQuestionContent || 'AskUserQuestion tool invoked';
          await markUserRequestAsWaitingFeedback(requestId, feedbackContent);
          feedbackDetected = true;
          console.log(`[ClaudeService] 🔔 Feedback detected via AskUserQuestion hook for requestId: ${requestId}`);
          timelineLogger.logSDK(projectId, 'Feedback detected via AskUserQuestion hook', 'info', requestId, undefined, 'sdk.feedback_detected.hook').catch(() => { });
        } catch (err) {
          console.warn('[ClaudeService] Failed to mark AskUserQuestion feedback:', err);
        }
      } else {
        // Fallback: text-based heuristic for cases where Claude asks in free text without using the tool
        try {
          const { isWaitingForFeedback } = await import('@/lib/services/feedback-detector');
          const { markUserRequestAsWaitingFeedback } = await import('@/lib/services/user-requests');
          ...
          if (lastContent && isWaitingForFeedback(lastContent)) {
            await markUserRequestAsWaitingFeedback(requestId, lastContent);
            feedbackDetected = true;
            console.log(`[ClaudeService] 🔔 Feedback detected via text heuristic (fallback) for requestId: ${requestId}`);
            timelineLogger.logSDK(projectId, 'Feedback detected via text heuristic (fallback)', 'info', requestId, undefined, 'sdk.feedback_detected.text').catch(() => { });
          }
        } catch (err) {
          console.warn('[ClaudeService] Text-based feedback detection failed, falling back to completed:', err);
        }
      }
    }
    if (!feedbackDetected) {
...
**禁止执行任何删除操作。** 如用户要求清理文件，请使用移动到指定文件夹（如 _trash）的方式替代删除。`;
      ...
      // 检查是否有员工专属的 system_prompt_plan（优先于通用项目类型提示词）
      const employeeId = (project as any)?.employee_id as string | undefined;
      let employeePlanPrompt: string | undefined;

      if (employeeId) {
        const { getEmployeeById } = await import('@/lib/services/employee-service');
        const employee = await getEmployeeById(employeeId);
        if (employee?.system_prompt_plan?.trim()) {
          employeePlanPrompt = employee.system_prompt_plan;
          console.log(`[ClaudeService] 👤 Using employee plan prompt for: ${employee.name}`);
        }
      }

      if (employeePlanPrompt) {
        // 使用员工专属规划提示词，包裹在 buildPlanningSystemPrompt 中
        const { buildPlanningSystemPrompt } = await import('@/lib/config/prompts');
        const { loadGlobalSettings } = await import('@/lib/services/settings');
        const settings = await loadGlobalSettings();
        systemPromptText = buildPlanningSystemPrompt(employeePlanPrompt, settings.ai_services);
        console.log(`[ClaudeService] 🎯 Using employee-specific plan prompt (${systemPromptText.length} chars)`);
      } else {
