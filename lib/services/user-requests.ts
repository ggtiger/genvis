 ...
 * Only checks the most recent request within time window to avoid stale data
 */
export async function getActiveTaskForProject(projectId: string, timeWindowMinutes = 5): Promise<ActiveTaskInfo | null> {
  ...
  // Time window filter to exclude stale/orphan tasks
  const timeWindowAgo = Date.now() - timeWindowMinutes * 60 * 1000;
  const taskCreatedAt = latestTask.createdAt ? new Date(latestTask.createdAt).getTime() : 0;
  if (taskCreatedAt < timeWindowAgo) {
    return null;
  }
...
// ========== Feedback Question Storage (in-memory) ==========

/** 内存 Map 存储反馈问题内容，key 为 requestId */
const feedbackQuestions = new Map<string, string>();

/**
 * 将 UserRequest 标记为 waiting_feedback 状态，并存储反馈问题内容。
 */
export async function markUserRequestAsWaitingFeedback(
  id: string,
  feedbackQuestion?: string
): Promise<void> {
  await updateStatus(id, 'waiting_feedback' as UserRequestStatus, {
    setCompletionTimestamp: false,
  });
  if (feedbackQuestion) {
    feedbackQuestions.set(id, feedbackQuestion);
  }
}

/**
 * 获取 UserRequest 对应的反馈问题内容。
 */
export function getFeedbackQuestion(id: string): string | undefined {
  return feedbackQuestions.get(id);
}

/**
 * 清除 UserRequest 对应的反馈问题内容。
 */
export function clearFeedbackQuestion(id: string): void {
