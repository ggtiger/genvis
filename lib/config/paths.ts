 ...
 * Priority: GENVIS_RESOURCES_PATH (standalone) > resourcesPath (Electron) > cwd (dev)
 */
export function getBuiltinPythonPath(): string | null {
  try {
    ...
    // Priority 1: GENVIS_RESOURCES_PATH (passed from Electron main to standalone subprocess)
    const resourcesPath = process.env.GENVIS_RESOURCES_PATH;
    if (resourcesPath) {
      const pythonPath = path.join(resourcesPath, 'python-runtime', platformDir, 'bin', pythonBin);
      if (fs.existsSync(pythonPath)) {
        // console.log(`[PathConfig] ✅ Found builtin Python via GENVIS_RESOURCES_PATH: ${pythonPath}`);
        return pythonPath;
      }
    }
...
export function getClaudeCodeExecutablePath(): string {
  try {
    ...
    // Priority 2: GENVIS_RESOURCES_PATH (resource root passed from main process)
    const resourcesPath = process.env.GENVIS_RESOURCES_PATH;
    if (resourcesPath) {
      ...
      if (fs.existsSync(cliPath)) {
        // console.log(`[PathConfig] ✅ CLI found via GENVIS_RESOURCES_PATH: ${cliPath}`);
        return cliPath;
      }
    }
...
function getSkillsDirectory(): string {
  ...
  // Priority 2: GENVIS_RESOURCES_PATH (passed from Electron main to standalone subprocess)
  const resourcesPath = process.env.GENVIS_RESOURCES_PATH;
  if (resourcesPath) {
    const skillsPath = path.join(resourcesPath, 'skills');
    if (fs.existsSync(skillsPath)) {
      // console.log(`[PathConfig] ✅ Builtin skills via GENVIS_RESOURCES_PATH: ${skillsPath}`);
      return skillsPath;
    }
  }
...
function getBuiltinEmployeesPath(): string {
  ...
  if (resourcesPath) {
    ...
    if (fs.existsSync(builtinPath)) {
      // console.log(`[PathConfig] ✅ Builtin employees via GENVIS_RESOURCES_PATH: ${builtinPath}`);
      return builtinPath;
    }
  }
...
function getAIServicesDocsDirectory(): string {
  ...
  if (resourcesPath) {
    ...
    if (fs.existsSync(docsPath)) {
      // console.log(`[PathConfig] ✅ AI services docs via GENVIS_RESOURCES_PATH: ${docsPath}`);
      return docsPath;
    }
  }
