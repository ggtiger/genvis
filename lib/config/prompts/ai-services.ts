...
- **Env Vars:** \`GENVIS_ASR_SUBMIT_URL\`, \`GENVIS_ASR_QUERY_URL_TEMPLATE\`
...
# Submit
resp = requests.post(os.environ['GENVIS_ASR_SUBMIT_URL'], json={
...
query_url = os.environ['GENVIS_ASR_QUERY_URL_TEMPLATE'].replace('{task_id}', task_id)
while True:
...
export function buildAIServicesEnv(config: AIServicesConfig | undefined): Record<string, string> {
  ...
  if (config.asr?.enabled && config.asr.wanjie) {
    ...
    if (wanjie.base_url) {
      env.GENVIS_ASR_BASE_URL = wanjie.base_url;
    }
    if (wanjie.submit_url) {
      env.GENVIS_ASR_SUBMIT_URL = wanjie.submit_url;
    }
    if (wanjie.query_url_template) {
      env.GENVIS_ASR_QUERY_URL_TEMPLATE = wanjie.query_url_template;
    }
  }

  // Also inject docs path so subprocess can access it
  env.GENVIS_AI_SERVICES_DOCS_PATH = AI_SERVICES_DOCS_PATH;
